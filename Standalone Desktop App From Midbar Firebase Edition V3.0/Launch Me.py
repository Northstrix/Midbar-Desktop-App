"""
This is a standalone desktop app from Midbar Firebase Edition V3.0
Original Notice:
Midbar
...
conn = sqlite3.connect('midbar_data.db')
# Create a cursor object
cursor = conn.cursor()
cursor.execute('''CREATE TABLE IF NOT EXISTS bootleg_fs 
                  (fname text, content TEXT)''')
    ...
    def __init__(self):
        ...
        for frame_name in self.frames:
            ...
            if frame_name == "About":
                ...
                abl1 = customtkinter.CTkLabel(master=frame, text="Standalone Desktop Application", anchor="center", width=420, font=('Century Gothic', 20))
                abl1.place(x=50, y=40)
                abl2 = customtkinter.CTkLabel(master=frame, text="From Midbar Firebase Edition V3.0", anchor="center", width=420, font=('Century Gothic', 20))
                ...
                abl3.place(x=50, y=160)
                abet1 = customtkinter.CTkEntry(master=frame, width=420, font=('Century Gothic', 16), placeholder_text="github.com/Northstrix/Midbar-Desktop-App")
                abet1.place(x=50, y=210)
                ...
                abet3.place(x=60, y=380)
                abet3.configure(state='readonly')
...
def get_and_decrypt_note_title():
    global note_ttl_lbl
    extr_encr_ttl = read_file("N{}_ttl".format(pointer_for_notes))
...
def get_and_decrypt_phone_number_title():
    global phone_number_lbl
    extr_encr_ttl = read_file("P{}_ttl".format(pointer_for_phone_numbers))
    if extr_encr_ttl is None:
...
def get_and_decrypt_login_credential_title():
    global login_ttl_lbl
    extr_encr_ttl = read_file("L{}_ttl".format(pointer_for_logins))
    if extr_encr_ttl is None:
...
def get_and_decrypt_credit_card_credential_title():
    global credit_card_ttl_lbl
    extr_encr_ttl = read_file("C{}_ttl".format(pointer_for_credit_cards))
    if extr_encr_ttl is None:
...
def set_password(entry_text):
    ...
    encrypt_with_aes(array2)
    rest_aes_k()
    write_file({"mpass" : string_for_data})
    create_main_window()
    
def check_password(entry_text, extr_encr_hash):
    string_salt = read_file("salt")
    ...
    hmackey = derived_key[32:64]
    bytes_for_mp = derived_key[64:]
    extr_encr_pass_h = read_file("mpass")
...
def edit_login():
    if pointer_for_logins == 0:
        ...
        extr_encr_ttl = read_file("L{}_ttl".format(pointer_for_logins))
        extr_encr_usn = read_file("L{}_usn".format(pointer_for_logins))
        extr_encr_psw = read_file("L{}_psw".format(pointer_for_logins))
        extr_encr_wbs = read_file("L{}_wbs".format(pointer_for_logins))
        extr_encr_tag = read_file("L{}_tag".format(pointer_for_logins))
...
def set_login_to_firebase(entered_title, entered_username, entered_password, entered_website):
    ...
    write_file({"L{}_ttl".format(pointer_for_logins) : string_for_data})
    encrypt_string_with_aes_in_cbc(entered_username)
    write_file({"L{}_usn".format(pointer_for_logins) : string_for_data})
    encrypt_string_with_aes_in_cbc(entered_password)
    write_file({"L{}_psw".format(pointer_for_logins) : string_for_data})
    encrypt_string_with_aes_in_cbc(entered_website)
    write_file({"L{}_wbs".format(pointer_for_logins) : string_for_data})
    ...
    write_file({"L{}_tag".format(pointer_for_logins) : string_for_data})
    update_cred_values_login()
...
def delete_login():
    if (pointer_for_logins == 0):
        ...
        if confirm_delete:
            # Delete the files
            delete_file("L{}_ttl".format(pointer_for_logins))
            delete_file("L{}_usn".format(pointer_for_logins))
            delete_file("L{}_psw".format(pointer_for_logins))
            delete_file("L{}_wbs".format(pointer_for_logins))
            delete_file("L{}_tag".format(pointer_for_logins))
            update_cred_values_login()
...
def edit_credit_card():
    if pointer_for_credit_cards == 0:
        ...
        # Extract encrypted data from the database
        extr_encr_ttl = read_file("C{}_ttl".format(pointer_for_credit_cards))
        extr_encr_hld = read_file("C{}_hld".format(pointer_for_credit_cards))
        extr_encr_nmr = read_file("C{}_nmr".format(pointer_for_credit_cards))
        extr_encr_exp = read_file("C{}_exp".format(pointer_for_credit_cards))
        extr_encr_cvn = read_file("C{}_cvn".format(pointer_for_credit_cards))
        extr_encr_pin = read_file("C{}_pin".format(pointer_for_credit_cards))
        extr_encr_zip = read_file("C{}_zip".format(pointer_for_credit_cards))
        extr_encr_tag = read_file("C{}_tag".format(pointer_for_credit_cards))
...
def set_credit_card_to_firebase(entered_title, entered_cardholder, entered_card_number, entered_expiry, entered_cvn, entered_pin, entered_zip_code):
    encrypt_string_with_aes_in_cbc(entered_title)
    write_file({"C{}_ttl".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_cardholder)
    write_file({"C{}_hld".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_card_number)
    write_file({"C{}_nmr".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_expiry)
    write_file({"C{}_exp".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_cvn)
    write_file({"C{}_cvn".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_pin)
    write_file({"C{}_pin".format(pointer_for_credit_cards): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_zip_code)
    write_file({"C{}_zip".format(pointer_for_credit_cards): string_for_data})
    ...
    write_file({"C{}_tag".format(pointer_for_credit_cards): string_for_data})
    update_cred_values_credit_card()
    messagebox.showinfo("Midbar | מדבר", "Slot content modified successfully!")
...
def delete_credit_card():
    if pointer_for_credit_cards == 0:
        ...
        if confirm_delete:
            # Delete the files
            delete_file("C{}_ttl".format(pointer_for_credit_cards))
            delete_file("C{}_hld".format(pointer_for_credit_cards))
            delete_file("C{}_nmr".format(pointer_for_credit_cards))
            delete_file("C{}_exp".format(pointer_for_credit_cards))
            delete_file("C{}_cvn".format(pointer_for_credit_cards))
            delete_file("C{}_pin".format(pointer_for_credit_cards))
            delete_file("C{}_zip".format(pointer_for_credit_cards))
            delete_file("C{}_tag".format(pointer_for_credit_cards))
            update_cred_values_credit_card()
            messagebox.showinfo("Midbar | מדבר", "Record deleted successfully!")
        else:
            messagebox.showinfo("Midbar | מדבר", "Operation has been cancelled by user")
...
def edit_note():
    if pointer_for_notes == 0:
        ...
        # Extract encrypted data from the database
        extr_encr_ttl = read_file("N{}_ttl".format(pointer_for_notes))
        extr_encr_cnt = read_file("N{}_cnt".format(pointer_for_notes))
        extr_encr_tag = read_file("N{}_tag".format(pointer_for_notes))
...
def set_note_to_firebase(entered_title, entered_content):
    encrypt_string_with_aes_in_cbc(entered_title)
    write_file({"N{}_ttl".format(pointer_for_notes): string_for_data})
    encrypt_string_with_aes_in_cbc(entered_content)
    write_file({"N{}_cnt".format(pointer_for_notes): string_for_data})
    title_content = entered_title + entered_content
    encrypt_tag_with_aes_in_cbc(title_content)
    write_file({"N{}_tag".format(pointer_for_notes): string_for_data})
    update_cred_values_note()
    messagebox.showinfo("Midbar | מדבר", "Slot content modified successfully!")
...
def delete_note():
    if pointer_for_notes == 0:
        ...
        if confirm_delete:
            # Delete the files
            delete_file("N{}_ttl".format(pointer_for_notes))
            delete_file("N{}_cnt".format(pointer_for_notes))
            delete_file("N{}_tag".format(pointer_for_notes))
            update_cred_values_note()
            messagebox.showinfo("Midbar | מדבר", "Record deleted successfully!")
        else:
            messagebox.showinfo("Midbar | מדבר", "Operation has been cancelled by user")
...
def edit_phone_number():
    if pointer_for_phone_numbers == 0:
        ...
        # Extract encrypted data from the database
        extr_encr_num = read_file("P{}_ttl".format(pointer_for_phone_numbers))
        extr_encr_name = read_file("P{}_cnt".format(pointer_for_phone_numbers))
        extr_encr_tag = read_file("P{}_tag".format(pointer_for_phone_numbers))
...
def set_phone_number_to_firebase(entered_number, entered_contact_name):
    ...
    write_file({"P{}_ttl".format(pointer_for_phone_numbers): string_for_data})
    clear_variables()
    encrypt_string_with_aes_in_cbc(entered_contact_name)
    write_file({"P{}_cnt".format(pointer_for_phone_numbers): string_for_data})
    clear_variables()
    encrypt_tag_with_aes_in_cbc(entered_number + entered_contact_name)
    write_file({"P{}_tag".format(pointer_for_phone_numbers): string_for_data})
    update_cred_values_phone_number()
    messagebox.showinfo("Midbar | מדבר", "Slot content modified successfully!")
...
def delete_phone_number():
    if pointer_for_phone_numbers == 0:
        ...
        if confirm_delete:
            # Delete the files
            delete_file("P{}_ttl".format(pointer_for_phone_numbers))
            delete_file("P{}_cnt".format(pointer_for_phone_numbers))
            delete_file("P{}_tag".format(pointer_for_phone_numbers))
            update_cred_values_phone_number()
            messagebox.showinfo("Midbar | מדבר", "Record deleted successfully!")
        else:
            messagebox.showinfo("Midbar | מדבר", "Operation has been cancelled by user")
...
def delete_file(fname_value):
    # Delete the record from the database
    cursor.execute("DELETE FROM bootleg_fs WHERE fname = ?", (fname_value,))
    # Commit the transaction
    conn.commit()

def read_file(fname):
    return try_reading_rcrd(fname)

def write_file(data):
    if isinstance(data, dict):
        for fname, content in data.items():
            read_value = try_reading_rcrd(fname)
            if read_value is None:
                add_record_to_database(fname, content)
            else:
                update_record_in_database(fname, content)

def add_record_to_database(fname_value, content_value):
    # Insert the record into the database
    cursor.execute("INSERT INTO bootleg_fs (fname, content) VALUES (?, ?)",
                    (fname_value, content_value))
    # Commit the transaction
    conn.commit()
    
def update_record_in_database(fname_value, new_content_value):
    # Update the record in the database
    cursor.execute("UPDATE bootleg_fs SET content = ? WHERE fname = ?", (new_content_value, fname_value))
    # Commit the transaction
    conn.commit()

def try_reading_rcrd(fname_value):
    cursor.execute("SELECT content FROM bootleg_fs WHERE fname=?", (fname_value,))
    content = cursor.fetchone()
    return content[0] if content else None

if __name__ == "__main__":
    customtkinter.set_appearance_mode("light")  # Modes: system (default), light, dark
    #write_file({"Test File" : "Content"})
    #print(read_file("Should return None"))
    #print(read_file("Test File"))
    #print(read_file("mpass"))
    extr_encr_hash = read_file("mpass")
    if extr_encr_hash is None:
